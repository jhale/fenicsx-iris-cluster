...
cd ${BUILD_DIR} && \
   git clone -b v0.6.0 --single-branch --depth 1 https://github.com/fenics/basix.git && \
   cd basix/cpp && \
...
python3 -m pip install -v --no-cache-dir git+https://github.com/FEniCS/ufl.git@2023.1.0
python3 -m pip install -v --no-cache-dir git+https://github.com/FEniCS/ffcx.git@v0.6.0

unset I_MPI_PMI_LIBRARY # Necessary if running in interactive session
cd ${BUILD_DIR} && \
   git clone -b v0.6.0 --single-branch --depth 1 https://github.com/fenics/dolfinx.git && \
   cd dolfinx/cpp && \
